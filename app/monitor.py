...
from .parser import parse_migration, parse_activity
...
class WSMonitor:
    ...
    async def _subscribe(self, ws, pubkey: str) -> int:
        self._req += 1
            ...
            try:
                async with websockets.connect(self.ws_url, ping_interval=20, ping_timeout=20, close_timeout=5) as ws:
                    ...
                    while time.monotonic() < deadline:
                        ...
                        try:
                            sig=await asyncio.wait_for(self._next_signature(ws), timeout=remaining)
