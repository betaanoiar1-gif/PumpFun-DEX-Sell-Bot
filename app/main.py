    ...
    while True:
        try:
            async with websockets.connect(cfg.ws_url, ping_interval=20, ping_timeout=20, close_timeout=5) as ws:
                ...
                sub=await ws.recv()
                log.info("Pump.fun logsSubscribe connected: %s", sub)
                while True:
                    raw=await ws.recv()
                    msg=json.loads(raw)
                    ...
                    if not pool:
                        ...
                        log.info("NO_POOL_FOUND mint=%s migration=%s",mint,sig)
                        continue
                    run.pool=pool
                    run.state=State.POOL_FOUND
                    ...
                    if run.state==State.EXECUTABLE_ACTIVITY_DETECTED:
                        ...
                        # Intentionally no wallet, no quote submission and no signer.
            ...
            log.warning("Pump.fun websocket disconnected: %s; reconnecting",e)
            await asyncio.sleep(1)
        except Exception:
            log.exception("Unexpected stream error; reconnecting")
...
async def main():
    ...
    await migration_stream(cfg,rpc)
